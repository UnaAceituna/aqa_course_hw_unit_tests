...
function mapThisArray<T, U> (array: T[], callback: MapCallback<T, U>): U[] {
    const myArr: U[] = [];
    for (let i = 0; i < array.length; i++) {
        myArr.push(callback(array[i], i, array))
    }
    return myArr
}

const arrayOfNumbers = [1,2,3,4,5]
const customMap = mapThisArray(arrayOfNumbers, (elem, index) => elem * index)
